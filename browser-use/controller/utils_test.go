...
func TestValidateSchema(t *testing.T) {
	schemaString := GenerateSchema("InputTextAction", &InputTextAction{})
	err := ValidateSchema(schemaString, "{\"index\": 3, \"text\": \"GPU compiler\"}")
	if err != nil {
		t.Error(err)
	...
	err = ValidateSchema(schemaString, `{"url": "https://www.google.com"}`)
	if err != nil {
		t.Error(err)
	...
	schemaString = GenerateSchema("ClickElementAction", &ClickElementAction{})
